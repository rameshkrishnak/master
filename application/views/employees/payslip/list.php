    ...
    <script type="text/javascript" src="<?php echo base_url();?>assets/js/script/bootstrap-multiselect.js"></script>
	...
	//alert(date_history);
	//for mail option selected_month_mail
	var selected_month_mail     =  $("#selected_month_mail").val();	
	var selected_month_mail_to  =  $("#selected_month_mail_to").val();
	...
	$("#selected_month_view_to").val(date_history_to);
	//for mail
	$("#selected_month_mail").val(date_history);
	$("#selected_month_mail_to").val(date_history_to);
...
jQuery('#date_history_to').on("change",function(){
	
	var date_history_to=$.trim($("#date_history_to").val());
	//alert('date'+date_history);
	var d = new Date();	
	//var date_history=$("#date_history").val();
	//alert((d.getMonth()+1));
	...
	//to date
	var date_history_to  =  $("#date_history_to").val();
	if(date_history_to!=current_mon)
	{
...
jQuery('#site_name_mail').multiselect({
    <?php  if ($this->session->userdata('user_group')!='2' && $this->session->userdata('user_group') !=3) { ?>
    includeSelectAllOption: true,
	<?php } ?>
    includeSelectAllIfMoreThan: 0,
   ...
   <div class="container top">
	...
	//echo '<br>'.base_url();
		...
		$count_emplpoyee=count($all_value);
		if($count_emplpoyee>0)
		{
			for($k=0;$k<count($all_value);$k++)
			{
				//echo count($data[$k]);
				//echo $data[$k][1]['employee_id_no'];
			for($l=0;$l<count($all_value[$k]);$l++)
			{			
				$curr_date_month               = $all_value[$k][$l]['curr_date_month'];	
				$this->email->clear();
				//echo 'hihi';
				//echo $row['curr_date_month'];
				$resultforSend ='';
				$filedata='';
				// if(isset($start_end_date))
					// {
					  ...
					  $start_date  = date('Y-m-01', strtotime($all_value[$k][$l]['curr_date_month']));
		              $end_date   = date('Y-m-t', strtotime($all_value[$k][$l]['curr_date_month']));
					  $ch = curl_init();
					  //$url='http://localhost:8080/attendance.php?start_date='.$start_date.'&end_date='.$end_date;
					  $url='http://mgt.nathanresearch.com/project_mgt/attendance.php?start_date='.$start_date.'&end_date='.$end_date.'&emp_id='.$all_value[$k][$l]['team_profile_employee_id'];
					  $curl = curl_init($url);
					...
					//  print_r($curl_response1);
					 ...
					 //  print_r($fullname_arr);
					  ...
					  // When we give the employee id here you can get the count(IN,Leave,SL) of employee status 
						 ...
						 $employee_id=$all_value[$k][$l]["team_profile_employee_id"];
		...
		$resultforSend .= '<div align="center"><table cellpadding="3" cellspacing="0" border="1" width="90%">
						  ...
						  <tr><td colspan="2"><b>Employee Name &nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp; </b>'.$all_value[$k][$l]["team_profile_full_name"].'</td><td><b>Employee ID :</b></td><td>'.$all_value[$k][$l]["team_profile_employee_id"].'</td></tr>
						  <tr><td colspan="1"><b>Designation &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: &nbsp;&nbsp;&nbsp;&nbsp;</b></td><td align="center" colspan="3"><b>Employee Attendance for Pay Period of '.date("M-y",strtotime($all_value[$k][$l]["curr_date_month"])).'</b></td></tr>
						  ...
						  <tr><td >Basic salary</td><td align="right">'.$all_value[$k][$l]["employ_basic_salary"].'</td><td>Income Tax</td><td colspan="2" align="right">'.$all_value[$k][$l]["employ_deduct_tax"].'</td></tr>
						  <tr><td>HRA</td><td align="right">'.$all_value[$k][$l]["employ_house_allow"].'</td><td>PF</td><td colspan="3" align="right">'.$all_value[$k][$l]["employ_deduct_provident"].'</td></tr>
						  <tr><td>Conveyance</td><td align="right" >'.$all_value[$k][$l]["employ_conveyance"].'</td><td colspan="3"></td></tr>
						  <tr><td>Medical </td><td align="right">'.$all_value[$k][$l]["employ_medical_allow"].'</td><td colspan="3"></td></tr>
						  <tr><td> Other Allowance</td><td align="right">'.$all_value[$k][$l]["employ_other_allow"].'</td><td colspan="3"></td></tr>
						  <tr><td>Total Earnings</td><td align="right">'.$all_value[$k][$l]["employ_gross_sal"].' </td><td>Total Deductions</td><td colspan="2" align="right">'.$all_value[$k][$l]["employ_total_deduct"].'</td></tr>
						   <tr></tr>
						    <tr><td>Net Pay </td><td align="right" >'.$all_value[$k][$l]["employee_net_salary"].'</td><td>Credit to </td><td colspan="2" align="center">Bank</td></tr>
							<tr></tr>
						...
						$mail_to     = $all_value[$k][$l]["team_profile_email"];
						$subject=$all_value[$k][$l]["team_profile_full_name"].' Payslip';
						$mail_content='test';
			...
			//echo '<b>Group Email sent successfully</b>';
		         	echo '<div class="alert alert-success">';
					echo '<a class="close" data-dismiss="alert">×</a>';
					echo '<strong>Group Email sent successfully</strong>';
					echo '</div>';
		}
		else
		{
	 ...
	 // print_r($test); 
      ...
      <div class="row">
		...
		<?php
		foreach($Review_employee as $getViewPayslip)
		{
			echo $getViewPayslip;
		}
          ...
          <div class="well">
              ...
              <?php
				...
				if ($this->session->userdata('user_group')!=2 && $this->session->userdata('user_group')!=3) {
				 echo '<tr id="payslip_toggle">
				<td><label for="female">Payslip</label> </td><td>
				';			  
     		    $attributes = array('class' => 'form-inline reset-margin', 'id' => 'myform');
				 echo form_open('employees/payslip', $attributes);
				...
				$attributes1 = array('class' => 'form-inline reset-margin', 'id' => 'myform1');
				  echo form_open('employees/payslip', $attributes1);
              ...
              $Regenerate_submit = array('name' => 'regenerate_submit', 'class' => 'btn btn-primary', 'value' => 'Delete for Payslip Regenerate');
				...
				?>
				 <select id="site_name" name="optionValue[]" multiple="multiple">
					
					<?php 
					  $all_employees = array('' => "Select");
					  foreach ($get_all_employees as $row)
					  {
						  echo '<option value="'.$row['team_profile_employee_id'].'">'.$row['team_profile_full_name'].'</option>';
				...
				<input type="hidden" name="selected_month_view_to" id="selected_month_view_to" value="<?php echo date('Y-m'); ?>">
				<?php
				echo '
				</td><td>';
				...
				echo form_open('employees/payslip', $attributes2);
				echo '</td>
				<td>';
				//echo form_dropdown('mail_employee_id', $all_employees_mail, set_value('mail_employee_id'), 'class="span2"');
				?>
				 <select id="site_name_mail" name="mail_group_employee_id[]" multiple="multiple">
				...
				<input type="hidden" name="selected_month_mail_to" id="selected_month_mail_to" value="<?php echo date('Y-m'); ?>">
				<!--<input type="hidden" name="selected_month_mail" id="selected_month_mail" value="<?php echo date('Y-m'); ?>">-->
				<?php
				echo '
				</td><td>';
